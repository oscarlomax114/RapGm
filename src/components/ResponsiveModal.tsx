...
export default function ResponsiveModal({
  ...
  if (mode === "sheet") {
    return (
      <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center sm:p-4" onClick={onClose}>
        <div
          className={`bg-neutral-950 w-full sm:${maxWidth} sm:rounded-lg rounded-t-xl shadow-lg max-h-[90vh] sm:max-h-[85vh] flex flex-col`}
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-0 sm:p-4" onClick={onClose}>
      <div
        className={`bg-neutral-950 w-full h-full sm:h-auto sm:${maxWidth} sm:rounded-lg sm:shadow-lg sm:max-h-[85vh] flex flex-col overflow-hidden`}
        onClick={(e) => e.stopPropagation()}
      >
        {children}
      </div>
